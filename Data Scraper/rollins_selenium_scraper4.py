...
def wait_for_modal_data(driver, max_wait=5):
    """Wait for the booking details modal to finish loading."""
...
def force_close_modal(driver):
    """Force close any open modal by removing it from the DOM."""
...
def extract_bookings(driver, date):
    ...
    for i in range(total):
        try:
            ...
            # Click event block via JS by index
            ...
            details = wait_for_modal_data(driver, max_wait=5)
    ...
    log.info(f"    Done: {target_count} target, {skip_count} skipped, {error_count} errors")
    return records
    ...
    try:
        while current <= end_date:
            ...
            if navigate_to_date(driver, current):
                ...
                log.info(f"  -> {len(records)} target bookings collected (total so far: {len(all_records)})")
